...
def main():
    ...
    forward_command_Button = tk.Button(text="forward", command=send_forward)
    forward_command_Button.pack()

    back_command_Button = tk.Button(text="back", command=send_back)
    back_command_Button.pack()  # Pack the button into the window for display

    left_command_Button = tk.Button(text="left", command=send_left)
    left_command_Button.pack()  # Pack the button into the window for display
...
def socket_thread():
    ...
    cybot = cybot_socket.makefile("rbw",
                                  buffering=0)  # makefile creates a file object out of a socket:  https://pythontic.com/modules/socket/makefile
    # TCP Socket END
    ...
    while send_message != 'quit\n':
        ...
        if send_message == "m\n":
            ...
            file_object.close()
            polarScan()# Important to close file once you are done with it!!
        else:
