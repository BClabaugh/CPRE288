...
filename = 'sensor-scan.txt'  # Name of file containing mock cybot sensor scan data
...
while True:  # Loop recieving/sending data from/to client until client disconnects, or error causes data to be 0 (i.e., no data)
    ...
    # Check if a sensor scan command has been sent
        ...
        file_object = open(full_path + filename,
                           'r')  # Open the file: file_object is just a variable for the file "handler" returned by open()
        ...
        while (sensor_data != "END\n"):  # Collect sensor data until "END" recieved
            ...
            conn.send(
                sensor_data.encode())  # Send a line of sensor data to client. Convert from String to bytes (i.e., encode)
