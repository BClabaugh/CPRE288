...
// function to scan the CyBot surroundings in Lab 3
//void scan_path_old(cyBOT_Scan_t *scan_data, full_scan_data_t *full_scan_data){
...
//  }
//  find_objects_ping(full_scan_data);
//  uart_sendChar('\r');
//  uart_sendChar('\n');
...
// function to scan the CyBot surroundings in Lab 7
...
//    for(j=0; j < 3; j+=1){
//      cyBOT_Scan(i, scan_data);
//      scan_IR_ave += scan_data -> IR_raw_val;
//    }
//    scan_IR_ave = (scan_IR_ave / 3);
...
void find_objects_pos(full_scan_data_t *full_scan_data){
  ...
  for(i=0; i <= 90; i+=1)
  {
    ...
    if(full_scan_data -> IR_val_arr[i] > 75 || i==0 || i==1 || i==2){
      continue;
    }
    //object first detection
      ...
      for(j=i+1; j<=90; j+=1){
        int h = j;
        if((full_scan_data -> IR_val_arr[j] > 75 && h - i > 3) || j == 90){
...
void find_objects_full(full_scan_data_t *full_scan_data){
  ...
  for(k=0; k <= 7; k+=1){
    if(full_scan_data -> obj_num[k] != 0){
      ...
      //full_scan_data -> obj_width[k] = ( full_scan_data -> sound_dist_arr[middle_angle_array] )*( 2 * angle_difference );
      //full_scan_data -> obj_width[k] = ( angle_difference / 360.0 )*( full_scan_data -> sound_dist_arr[middle_angle_array] )*2*3.1415926;
...
int auto_driving(full_scan_data_t *self, oi_t *sensor_data, cyBOT_Scan_t *scan_data, int sml_obj){
    ...
    //move_forward_spd(sensor_data, (10*((self -> obj_IR_dist[sml_obj]) - 7)), 50);
    int tog = 0;
    tog = auto_forward(sensor_data, (10*((self -> obj_dist[sml_obj]) - 15)), 75);
    if(tog == 1){
        return 0;
    }

    // rescan if bump sensor activated while driving
    if(tog == 2){
        scan_path(scan_data, self);
        scan_objs(scan_data, self);
        int sml_obj_2 = 0;
        sml_obj_2 = smallest_object(self, scan_data);
        controlled_driving(self, sensor_data, scan_data, sml_obj_2);
    }
